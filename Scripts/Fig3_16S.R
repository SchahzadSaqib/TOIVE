...
mycols <- viridis::viridis(5, 
  ...
  base::append(PNWColors::pnw_palette(n = 5, 
                                      name = "Winter")) %>%
  base::append(PNWColors::pnw_palette(n = 3, 
                                      name = "Mushroom")) %>%
...
stack_bar_summ <- ggplot2::ggplot(p2, aes(x = CsCtrl, 
                                          y = Abundance, 
                                          fill = taxa)) +
...
for (cdl in 1:length(confounder_list)) {
  ...
  for (fct in 1:length(assign_names)) {
    ...
    ## Grouptest 
    gt_res <- try({GroupTest_mod(species.table = Toive_phy_16S,
                                 meta = Toive_phy_16S,
                                 group = var, 
                                 group_name = "Case (1) vs Control (2)",
                                 compare.to = "2", 
                                 dir_for_res = here::here("Results",
                                                          "16S",
                                                          "mare", 
                                                          "Case_vs_Control"),
    ...
    gt_summ_cmp <- gt_res %>%
      ...
      dplyr::ungroup() %>%
      dplyr::mutate(variable = str_extract(comp, pattern = paste(levels(factor(plot_data[,var])), 
                                                                 collapse = "|"))) %>%
...
## construct viobox
gt_viobox <- ggplot2::ggplot(data = p2, aes(x = CsCtrl,
                                            y = Abundance,
                                            fill = CsCtrl)) +
  ...
  gghalves::geom_half_violin(mapping = aes(colour = CsCtrl),
                             trim = F, 
                             side = "l",
  ...
  gghalves::geom_half_boxplot(mapping = aes(colour = CsCtrl),
                              side = "r", 
                              nudge = .05,
                              size = .3, 
                              colour = "grey70",
  ...
  ggplot2::facet_grid(factor(split, 
                             levels = c("Endometrium",
                                        "Vagina",
                                        "Pooled"))
                      ~factor(taxa, levels = c("Lactobacillus crispatus", 
...
p <- ggpubr::as_ggplot(g) + 
  cowplot::draw_plot_label(label = c("A)", 
                                     "B)", 
                                     "C)", 
                                     "D)", 
                                     "E)"), 
...
ggplot2::ggsave(path = path_to_fig,
                plot = p,
