...
p1 <- plot_data %>%
  dplyr::select(SampleN:CsCtrl, ends_with("_prc")) %>%
  dplyr::mutate(CsCtrl = case_when((CsCtrl == 1) ~ "Case",
                                   (CsCtrl == 2) ~ "Control")) %>%
...
# define species colours
mycols <- viridis::viridis(5, option = "mako", 
                           begin = 0.30) %>%
  ...
  base::append(PNWColors::pnw_palette(n = nlevels(p1$taxa) - 10, 
                                      name = "Shuksan")) %>%
  purrr::set_names(levels((p1$taxa))) %>%
  base::replace("Others", scico::scico(1, palette = "grayC", 
                                       begin = 0.65)) 
...
pcmpl <- p1 %>%
  ...
  dplyr::mutate(taxa_abd = case_when((L.crispatus_prc >= 0.50 | is.na(L.crispatus_prc)) ~ 4,
                                     (L.iners_prc >= 0.50) ~ 3,
                                     (G.vag_prc >= 0.50) ~ 2,
                                     TRUE ~ 1)) %>%
...
stack_bar <- ggplot2::ggplot(p2, aes(x = CsCtrl, 
                                     y = Abundance)) +
